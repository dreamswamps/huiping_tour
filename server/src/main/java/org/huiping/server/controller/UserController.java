...
public class UserController {
    ...
    /** 查询当前登录用户资料。 */
    @GetMapping("/profile")
    public Result<User> getProfile(@CurrentUserId Long userId) {
        return Result.success("获取成功", userService.getProfile(userId));
    }
...
//    调用频次少的可怜，不必换成DTO
    ...
    public Result<List<UserAddress>> listAddress(@CurrentUserId Long userId) {
...
//    TODO 后续修改小程序代码，顺道把路径参数userId删了
    ...
    public Result<UserAddress> getAddressDetail(@CurrentUserId Long userId,
    ...
    public Result<UserAddress> addAddress(@CurrentUserId Long userId,
        ...
        return Result.success("添加成功", addressService.addAddress(userId, req));
    }
    ...
    public Result<UserAddress> updateAddress(@CurrentUserId Long userId,
                                             @PathVariable Long addressId,
                                             @RequestBody @Valid AddressRequest req) {
        return Result.success("保存成功", addressService.updateAddress(userId, addressId, req));
    }
