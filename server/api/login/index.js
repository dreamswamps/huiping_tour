...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    let newUserUid = null;
    try {
      const [rows] = await pool.query("SELECT * FROM users WHERE openid = ?", [
        openid,
      ]);
      if (rows.length > 0) {
        user = rows[0];
        if (user && (!user.uid || String(user.uid).trim() === "")) {
          ...
          try {
            await pool.query("UPDATE users SET uid = ? WHERE openid = ?", [
              fillUid,
              openid,
            ]);
            user.uid = fillUid;
        ...
        const nickname =
          displayName || "旅行者" + Math.floor(Math.random() * 10000);
    ...
    const numericId =
      user && user.id != null
        ...
        : null;
    const idOk =
      numericId != null && Number.isInteger(numericId) && numericId > 0;
    ...
    const token = jwt.sign({ userId: numericId, openid }, jwtConfig.secret, {
