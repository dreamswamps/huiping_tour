...
const { Product, ProductDetail, MallCart, Order } = require('../../models/mall');
...
router.put('/cart/sync', requireUserAuth, async (req, res) => {
  ...
  try {
    ...
    for (const row of normalized) {
      await conn.query(
        `INSERT INTO carts (user_id, product_id, product_name, product_price, product_thumb, quantity)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [userId, row.productId, row.name, row.price, row.thumb || null, row.qty]
      );
    }
    await conn.commit();
    ...
    await conn.rollback();
    const msg = e && e.code === 'ER_NO_REFERENCED_ROW_2' ? '存在无效的商品 id' : e.message;
...
router.post('/orders', requireUserAuth, async (req, res) => {
  ...
  productIds = [...new Set(productIds.map((x) => Number(x)).filter((n) => Number.isInteger(n) && n > 0))];
  ...
  try {
    ...
    if (lines.length === 0) {
      await conn.rollback();
      return res.status(400).json({ code: 400, message: '购物车为空或所选商品不在购物车中' });
    }
    ...
    const totalAmount =
      Math.round(items.reduce((s, it) => s + Number(it.price) * Number(it.quantity), 0) * 100) / 100;
    ...
    res.json({ code: 200, data: order ? order.toPublicJSON() : { id: lastId, orderNo } });
  } catch (e) {
    await conn.rollback();
...
router.post('/orders/:id/cancel', requireUserAuth, async (req, res) => {
  ...
  try {
    ...
    if (st === 2 || st === 3) {
      return res.status(400).json({ code: 400, message: '当前状态不可取消，请联系客服' });
    }
    if (st !== 0 && st !== 1) {
