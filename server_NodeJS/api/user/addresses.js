...
async function promoteFirstDefaultIfNone(userId) {
  ...
  if (first) {
    await pool.query("UPDATE user_addresses SET is_default = 1 WHERE id = ?", [
      first.id,
    ]);
  }
}
...
function register(router, requireUserAuth) {
  ...
  router.post("/:userId/addresses", requireUserAuth, async (req, res) => {
    ...
    const b = req.body || {};
    const receiverName =
      typeof b.receiverName === "string"
        ? b.receiverName.trim().slice(0, 50)
        : "";
    const receiverPhone =
      typeof b.receiverPhone === "string"
        ? b.receiverPhone.trim().slice(0, 20)
        : "";
    const province =
      typeof b.province === "string" ? b.province.trim().slice(0, 32) : "";
    const city = typeof b.city === "string" ? b.city.trim().slice(0, 32) : "";
    const district =
      typeof b.district === "string" ? b.district.trim().slice(0, 32) : "";
    const detailAddress =
    ...
    const label =
      typeof b.label === "string" && b.label.trim()
        ? b.label.trim().slice(0, 20)
        : null;
    let isDefault =
      b.isDefault === true || b.isDefault === 1 || b.isDefault === "1";
    ...
    if (
      ...
      return res
        .status(400)
        .json({ code: 400, message: "请填写完整的收货信息" });
    }
    ...
    async (req, res) => {
      ...
      const province =
        typeof b.province === "string"
          ? b.province.trim().slice(0, 32)
          : existing.province;
      const city =
        typeof b.city === "string" ? b.city.trim().slice(0, 32) : existing.city;
      const district =
        typeof b.district === "string"
          ? b.district.trim().slice(0, 32)
          : existing.district;
      const detailAddress =
        typeof b.detailAddress === "string"
          ? b.detailAddress.trim().slice(0, 255)
